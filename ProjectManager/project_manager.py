...
class ProjectManager(QWidget):
    ...
    def __init__(self, proj_dir, MainWindow):
       ...
       # self.proj_name_input.setReadOnly(True)
        ...
        self.input_Drawio_btn = QPushButton("Input Drawio")
        self.input_Drawio_btn.setFont(input_font)
        self.input_Drawio_btn.setStyleSheet(
            "QPushButton {background-color: white; color: black; border-radius: 5px; border-style: plain;padding; 10px; }"
            " QPushButton:pressed { background-color: rgb(250, 250, 250);  color: black; border-radius: 5px; border-style: plain;padding; 10px;}")
    ...
    def setup_ui(self):
        ...
        self.projDetailIpLayout.addWidget(self.input_Drawio_btn, 7, 2, 1, 1)
        ...
        self.input_Drawio_btn.clicked.connect(self.drawio_xmlReader)
   ...
   # def name_edit(self):
    ...
    def drawio_xmlReader(self):
        try:
            xml_file_path = QFileDialog.getOpenFileName(self, "Choose draw io file", self.proj_enviro,
                                                    filter="Drawio (*.drawio.xml)")
        ...
        for element in root.iter():
            if 'value' in element.attrib:
                result_array.append(element.attrib['value'])
            if 'label' in element.attrib:
                result_array.append(element.attrib['label'])
            if 'tags' in element.attrib:
                result_array.append(element.attrib['tags'])
            if 'link' in element.attrib:
                result_array.append(element.attrib['link'])

        result_array = [x for x in result_array if x != '']
        result_array = [element for i, element in enumerate(result_array) if i == 0 or element != result_array[i - 1]]
        result_array = [element for i, element in enumerate(result_array) if
                        i == 0 or not (element.endswith('_process') and
                                       result_array[i - 1].endswith(
                                           '_process'))]

        descriptionArray = [element for element in result_array if '<h1>' in element]
        result_array = [element for element in result_array if '<h1>' not in element]

        pattern = re.compile(r'>([^<]+)<')
        content_array = []
        for element in descriptionArray:
            matches = pattern.findall(element)
            content_array.extend(matches)

        print('Printing Psuedo Code From Text Box', content_array)

        temp_arrays = [[]]  # Initialize the list of lists
        p = 0  # Initialize the index for temp_arrays

        # Iterate over the elements of content_array
        for element in content_array:
            if '_process' in element:
                # Start a new sublist with the element containing '_process'
                temp_arrays.append([element])
                p += 1  # Increment the index for temp_arrays
            else:
                # Add elements to the current sublist
                temp_arrays[p].append(element)

        # Remove the empty sublist at the beginning
        temp_arrays = temp_arrays[1:]

        findDefault = []
        for t in range(len(temp_arrays)):
            for element in temp_arrays[t]:
                if 'Default' in element:
                    findDefault.append(element)
                    temp_arrays[t].remove(element)

        for t in range(len(temp_arrays)):
            for i in range(len(temp_arrays[t])):
                element = temp_arrays[t][i]
                if '&gt;' in element or '&nbsp;' in element:
                    ...
                    element = element.replace('&nbsp;', '')
                elif '_process' not in element:
                    element += '&amp;#10;'
                temp_arrays[t][i] = element

        self.default = []
        for element in findDefault:
        ...
        for t in range(len(temp_arrays)):
            for i in range(len(temp_arrays[t])):
                element = temp_arrays[t][i]
                if '_process' not in element and element != temp_arrays[t][-1]:
                    element += ' '
                temp_arrays[t][i] = element

        self.noteParagraph = []
        for w in range(len(temp_arrays)):
            start = '<note>'
            end = '</note>'
            for element in temp_arrays[w]:
                if '_process' in element:
                    self.noteParagraph.append(element)
                else:
                    start += element
            start += end
            self.noteParagraph.append(start)
        ...
        for element in result_array:
            if ' ' in element:
                ...
                part1 = parts[0]
                part2 = parts[1]
                new_array.append(part1)
                new_array.append(result_array[result_array.index(element) + 1])
                new_array.append(part1)
                new_array.append('Out' + result_array[result_array.index(element) - 1])
                new_array.append(part2)
                new_array.append(result_array[result_array.index(element) + 1])
                new_array.append(part2)
                result_array.pop(result_array.index(element) + 1)
                result_array.pop(result_array.index(element) + 1)
            else:
                new_array.append(element)

        result_array = new_array
        print("Printing All Values:", result_array)
        componentTags = []
        signalTags = []

        if "Component" in result_array:
            if "Signals" in result_array:
                index1 = result_array.index("Component")
                index2 = result_array.index("Signals")
                if index1 < index2:
                    for y in range(index1 + 1, index2):
                        componentTags.append(result_array[y])
                    for x in range(index2 + 2, len(result_array)):
                        signalTags.append(result_array[x])
                else:
                    for y in range(index2 + 1, index1):
                        signalTags.append(result_array[y])
                    for x in range(index1 + 1, len(result_array)):
                        componentTags.append(result_array[x])

        components = []
        self.signals = []
        for x in range(0, len(componentTags), 2):
            components.append(componentTags[x])

        for x in range(0, len(signalTags), 2):
            self.signals.append(signalTags[x])

        print("Printing All Components:", components)
        print("Printing All Signals:", self.signals)

        processSignals = []

        for i in range(0, len(signalTags), 4):
            processSignals.extend(signalTags[i:i + 2])

        self.save_xml()
        self.drawioSave()

        try:
            self.hdlgen_file_path = self.proj_folder_input.text()+"/" + self.proj_name + "/" + "HDLGenPrj" + "/" + self.proj_name + ".hdlgen"
        except FileNotFoundError:
            print("File not found. Please enter a valid file name.")

        with open(self.hdlgen_file_path, 'r') as hdlgen_file:
            content = hdlgen_file.read()
        try:
            check = self.check_for_entityIOPort(content)
        except ValueError as e:
            print(e)
        entityIOPort = self.add_entityIOPort(content)
        with open(self.hdlgen_file_path, 'w') as hdlgen_file:
            hdlgen_file.write(entityIOPort)
        print(f"<entityIOPorts> has been added to '{self.hdlgen_file_path}'.")

        with open(self.hdlgen_file_path, 'r') as hdlgen_file:
            content = hdlgen_file.read()
        name_signal, number_signal, type_signal = self.set_signals_structure(self.signals)
        signals_paragraph = self.set_signals_paragraph(name_signal, number_signal, type_signal)
        try:
            signalsWrite = self.add_signals(content, check, signals_paragraph)
            with open(self.hdlgen_file_path, 'w') as hdlgen_file:
                hdlgen_file.write(signalsWrite)
            print(f"<signals> has been added to '{self.hdlgen_file_path}'.")
        except ValueError as e:
            print(e)
        self.replace_entityIOPortsEnd(self.hdlgen_file_path)

        with open(self.hdlgen_file_path, 'r') as hdlgen_file:
            content = hdlgen_file.read()
        process_structure = self.set_process_structure(processSignals)
        process_paragraph = self.set_process_paragraph(process_structure)
        process = self.add_process(content, process_paragraph)
        with open(self.hdlgen_file_path, 'w') as hdlgen_file:
            hdlgen_file.write(process)
            print(f"Processes has been added to '{self.hdlgen_file_path}'.")
        self.drawio_load_proj_data(self.hdlgen_file_path)
        self.hdlgen_file = []
        self.hdlgen_file.append(self.hdlgen_file_path)
        self.hdlgen_file.append('Hi')
        self.drawio_close_project(self.hdlgen_file)

    def drawio_close_project(self, hdlgen_file):
        ...
        if self.MainWindow.isVisible() == False:
            ...
            self.window.show()
            self.window.open_drawio_project(hdlgen_file)
    ...
    def drawio_load_proj_data(self, load_proj_dir):
        print("Loading project from ", load_proj_dir)

        # Parsing the xml file
        data = minidom.parse(load_proj_dir)
        HDLGen = data.documentElement
        ...
        new_xml_path = load_proj_dir.split("/")
        ...
        if proj_loc != new_proj_loc:
            ...
            xml_str = os.linesep.join([s for s in xml_str.splitlines() if s.strip()])
            # Writing xml file
            with open(load_proj_dir, "w") as f:
                f.write(xml_str)
            if not os.path.isdir(proj_env):
                proj_env = os.path.dirname(new_proj_loc)
            new_proj_env = os.path.basename(os.path.normpath(proj_env))
    ...
    def add_entityIOPort(self,hdlgen_content):
        lines = hdlgen_content.split('\n')
        modified_lines = []
        for line in lines:
            modified_lines.append(line)
            if '<clkAndRst/>' in line:
                modified_lines.append('\n\t\t<entityIOPorts>')
        return '\n'.join(modified_lines)

    def check_for_entityIOPort(self,hdlgen_content):
        lines = hdlgen_content.split('\n')
        modified_lines = []
        for line in lines:
            modified_lines.append(line)
            if '<entityIOPorts>' in line:
                raise ValueError("Error: <entityIOPorts> is already present in the file.")
        return False

    def add_signals(self,hdlgen_content, is_entityIOPorts_not_present, paragraph):
        lines = hdlgen_content.split('\n')
        modified_lines = []
        for line in lines:
            modified_lines.append(line)
            if is_entityIOPorts_not_present:
                raise ValueError("Error: <entityIOPorts> is not present in the file.")
            if '<entityIOPorts>' in line:
                modified_lines.append(paragraph)
        return '\n'.join(modified_lines)

    def add_process(self,hdlgen_content, paragraph):
        lines = hdlgen_content.split('\n')
        modified_lines = []
        for line in lines:
            modified_lines.append(line)
    ...
    def replace_entityIOPortsEnd(self,file_path):
        new_sentence = '</entityIOPorts>'
        with open(file_path, 'r') as hdlGenfile:
            newContent = hdlGenfile.read()
        modified_content = newContent.replace('<entityIOPorts/>', new_sentence)
        with open(self.hdlgen_file_path, 'w') as hdlGenfile:
            hdlGenfile.write(modified_content)
        print(f"Occurrences of '<entityIOPorts/>' have been replaced with '</entityIOPorts>' in '{self.hdlgen_file_path}'.")

    def set_signals_structure(self,signals_array):
        signal_name = []
        signal_number = []
        signal_type = []

        for z in range(0, len(signals_array), 2):
            signal_name.append(signals_array[z])
            if self.signals[z + 1] == '1':
                signal_number.append(' bit')
                signal_type.append('single')
            else:
                signal_number.append('(' + str(int(self.signals[z + 1]) - 1) + ' downto 0)')
                signal_type.append('bus')

        seen_elements = set()
        unique_elements = []
        removed_indexes = []

        for i, element in enumerate(signal_name):
            if element not in seen_elements:
                unique_elements.append(element)
                seen_elements.add(element)
            else:
                removed_indexes.append(i)
        signal_name = unique_elements

        w = 0
        for o in range(0, len(removed_indexes)):
            signal_number.pop(removed_indexes[o - w])
            signal_type.pop(removed_indexes[o - w])
            w = w + 1

        return signal_name, signal_number, signal_type

    def set_process_structure(self,processSignalsArray):
        numProcesses = []
        for r in range(1, len(processSignalsArray), 2):
            numProcesses.append(processSignalsArray[r])
        unique_elements = []
        seen_elements = set()
        for element in numProcesses:
            if element not in seen_elements:
                unique_elements.append(element)
                seen_elements.add(element)
        numProcesses = unique_elements

        numProcesses = [[element] for element in numProcesses]
        for p in range(1, len(processSignalsArray), 2):
            for d in range(0, len(numProcesses)):
                if processSignalsArray[p] == numProcesses[d][0]:
                    numProcesses[d].append(processSignalsArray[p - 1])
        return numProcesses

    def set_signals_paragraph(self,name, number, types):
        paragraph = ''
        indent2 = '\n\n\t\t\t'
        indent4 = '\n\n\t\t\t\t'
        put = []
        for i in range(0, len(name)):
            ...
            if paragraph == '':
                ...
                indent3 = '\n\n\t\t\t'
            paragraph = paragraph + indent3 + '<signal>' + indent4 + '<name>' + name[
                i] + '</name>' + indent4 + '<mode>' + mode + '</mode>' + indent4 + '<type>' + types[i] + number[
                            i] + '</type>' + indent4 + '<description>to be completed</description>' + indent2 + '</signal>'
        return paragraph

    def set_process_paragraph(self,signaling):
        indent = '\n\n\t\t\t\t'
        paragraph = '\t\t\t\t'
        signalArray = []
        outputParagraph = ''
        for i in range(0, len(signaling)):
            for n in range(0, len(signaling[i])):
                if 'Out' in signaling[i][n]:
                    outputParagraph = '<defaultOutput>' + signaling[i][n] + ',' + self.default[i] + '</defaultOutput>'
                elif '_process' in signaling[i][n]:
                    labelParagraph = '<label>' + signaling[i][n] + '</label>'
                    processCheck = signaling[i][n]
                else:
                    signalParagraph = '<inputSignal>' + signaling[i][n] + '</inputSignal>'
                    signalArray.append(signalParagraph)
            paragraph = paragraph + '\n\t\t\t<process>' + indent + labelParagraph
            for element in signalArray:
                paragraph = paragraph + indent + element
            signalArray = []
            if self.noteParagraph != []:
                for element in self.noteParagraph:
                    if element == processCheck:
                        outputParagraph += indent + self.noteParagraph[self.noteParagraph.index(element) + 1]
                        break
            paragraph = paragraph + indent + outputParagraph + '\n\n\t\t\t' + '</process>' + '\n\t\t\t\t'
